...
public interface ThongKeRepository extends JpaRepository<HoaDon, Integer> {
        ...
        // + "AND hd.ngayThanhToan BETWEEN :ngayBatDau AND :ngayKetThuc " // Đảm bảo tên
        // tham số khớp
        // + "GROUP BY hd.ngayThanhToan "
        // + "ORDER BY hd.ngayThanhToan ASC")
        // List<SoLuongTourDaDatDTO> ThongKeTourTheoKhoangThoiGian(@Param("ngayBatDau")
        // Date ngayBatDau,
        // @Param("ngayKetThuc") Date ngayKetThuc);
        @Query("SELECT hd.ngayThanhToan, COUNT(DISTINCT cth.id) " +
                        "FROM HoaDon hd " +
                        "JOIN ChiTietHoaDon cth ON hd.id = cth.hoaDon.id " +
                        "WHERE hd.ngayThanhToan BETWEEN :startDate AND :endDate " +
                        "AND hd.trangThai = true " +
        ...
        List<Object[]> countTotalBookedToursByDate(@Param("startDate") Date startDate, @Param("endDate") Date endDate);
        ...
        // + "ORDER BY hd.ngayThanhToan ASC")
        // List<SoLuongNguoiDiTour>
        // findThongKeSoLuongNguoiDiTourTheoKhoangThoiGian(@Param("ngayBatDau") Date
        // ngayBatDau,
        // @Param("ngayKetThuc") Date ngayKetThuc);
        @Query("SELECT hd.ngayThanhToan, t.tenTour, COUNT(DISTINCT cth.id) " +
                        "FROM HoaDon hd " +
                        "JOIN ChiTietHoaDon cth ON hd.id = cth.hoaDon.id " +
                        "JOIN BienTheTour btt ON cth.bienTheTour.id = btt.id " +
                        "JOIN Tour t ON btt.tour.id = t.id " +
                        "WHERE hd.ngayThanhToan BETWEEN :startDate AND :endDate " +
                        "AND hd.trangThai = true " +
                        "GROUP BY hd.ngayThanhToan, t.tenTour " +
                        "ORDER BY hd.ngayThanhToan")
        List<Object[]> countTotalBookedToursByDateWithTour(@Param("startDate") Date startDate,
